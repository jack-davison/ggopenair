...
polarperc <-
  polar_percentile(marylebone, "nox") + theme_polar() + scale_color_manual(values = openair::openColours(n = 5))
# pollrose <-
#   pollutionrose(marylebone, "nox") + theme_polar(panel_ontop = FALSE) + scale_fill_manual(values = rev(openair::openColours(n = 7)))
# windrose <-
#   windrose(marylebone) + theme_polar(panel_ontop = FALSE) + scale_fill_manual(values = rev(openair::openColours(n = 4)))
...
<!-- ```{r} -->
<!-- windrose(marylebone, facet = "daylight") + -->
<!--   theme_polar(panel_ontop = FALSE) + -->
<!--   annotate_rose_text(y = 15, wd = "SE", size = 2.5) -->
<!-- ``` -->

<!-- The functionality to compare two sets of ws/wd data felt a bit misplaced in `pollutionRose()`, so `{ggopenair}` exports a separate `biasrose()` function. This also lets the plot better reflect what it is actually representing (no more N/S/E/W axis labels). -->
