...
#' according to \code{avg_time}). The time intervals are made up of bars split
...
#'
#' @param data A data frame containing the fields \code{date}, \code{pollutant}
#'   and a splitting variable \code{proportion}
...
#'   month"}.
#'
#'   Note that \code{avg_time} when used in \code{timeProp} should be greater
#'   than the time gap in the original data. For example, \code{avg_time =
#'   "day"} for hourly data is OK, but \code{avg_time = "hour"} for daily data
#'   is not.
#' @param facet \code{facet} determines how the data are split i.e. conditioned,
#'   and then plotted. The default is will produce a single plot using the
#'   entire data. Type can be one of the built-in types as detailed in
#'   \code{cutData} e.g. "season", "year", "weekday" and so on. For example,
#'   \code{facet = "season"} will produce four plots --- one for each season.
#'
#'   It is also possible to choose \code{facet} as another variable in the data
#'   frame. If that variable is numeric, then the data will be split into four
#'   quantiles (if possible) and labelled accordingly. If facet is an existing
