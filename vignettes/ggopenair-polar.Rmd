...
```{r allpolar, fig.width=10, fig.height=4}
polarplot <-
...
wrap_plots(polarplot, polarannulus, polarfreq, polarperc, pollrose, windrose)
```

`polarDiff()` and `polarCluster()` are still to be developed owing to developments required in `{openair}` itself.
