...
#'
#' This function is similar to [openair::timeAverage()], but responds to grouped
#' data frames (through [dplyr::group_by()]).
#'
#' @param data A data frame containing a date field. Can be class `POSIXct` or
#'   `Date`.
#' @param avg_time This defines the time period to average to. Can be “sec”,
...
#'   fill = TRUE.
#' @param data_thresh The data capture threshold to use (%). A value of zero
...
#' @param ... Other arguments to pass to [openair::timeAverage()].
#' @param type Not used. Please use [dplyr::group_by()].
#'
